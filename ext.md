...
Как работает: Коммутаторы соединяются между собой специальными высокоскоростными стековыми кабелями через выделенные для этого порты на задней панели, либо, как показано на слайде 9, через обычные линейные (Ethernet) порты. Один коммутатор в стеке становится главным (master), а остальные — подчиненными (slave).
...
Примеры технологий: В презентации упоминаются проприетарные названия этой технологии: Cisco StackWise, HPE IRF, Juniper Virtual Chassis.
...
Как работает: Как показано на слайде 12, для работы MC-LAG требуется несколько соединений между коммутаторами:
...
Технология	Control Plane	Управление	Масштаб
Стекирование	Единый	Централизованное	Малый (до 8-10 устройств)
MC-LAG	Раздельный	Децентрализованное	Малый (2 устройства)
Фабрика	Раздельный	Децентрализованное	Крупный (сотни устройств)
2. Multihoming
Что это такое?
Как определено в презентации, Multihoming — это подключение одного конечного устройства (например, сервера) несколькими физическими линками к нескольким разным сетевым устройствам для обеспечения отказоустойчивости и увеличения пропускной способности.
...
Презентация выделяет три протокола для управления этим процессом:
...
Заключение:
Таким образом, для создания отказоустойчивых и высокопроизводительных сетей вместо одиночных коммутаторов используются технологии их объединения. Стекирование предлагает простоту управления в небольших инсталляциях. MC-LAG обеспечивает отказоустойчивость с сохранением независимости устройств. А IP-фабрики являются стандартом для крупных, масштабируемых сетей. Все эти технологии являются необходимым условием для реализации Multihoming — подключения конечных устройств к сети по отказоустойчивой схеме с использованием протоколов агрегации, таких как LACP.

# 14 Процесс маршрутизации пакетов
1. Что такое маршрутизация и ее задачи
Как четко определено в презентации, маршрутизация — это процесс передачи IP-пакетов между интерфейсами L3-устройства (маршрутизатора) на основании IP-адресов.
...
Центральным элементом, на основе которого маршрутизатор принимает решения, является таблица маршрутизации. Презентация выделяет два типа таблиц:
...
Таким образом, процесс маршрутизации — это сложный, но строго регламентированный алгоритм. Он начинается с поиска наиболее точного маршрута в таблице на основе Longest Prefix Match, затем переходит к определению MAC-адреса следующего узла с помощью ARP и завершается формированием и отправкой нового L2-кадра. Этот цикл, повторяющийся на каждом маршрутизаторе, и обеспечивает глобальную связность в IP-сетях.
...
Внутренняя динамическая маршрутизация (IGP - Interior Gateway Protocol), как указано в презентации, используется для обмена маршрутной информацией внутри одной автономной системы (AS). Автономная система — это совокупность сетей под единым техническим и административным управлением (например, сеть одного интернет-провайдера или крупной компании).
...
Принцип "Маршрутизация по слухам": Как показано на слайдах 18-20, маршрутизатор не знает всей топологии сети. Он знает только о своих прямых соседях и о маршрутах, которые ему эти соседи "рассказали". Он доверяет их информации, добавляя к полученной метрике свою собственную.
...
Принцип "Каждый строит свою карту": Как показано на слайдах 21-24, каждый маршрутизатор в сети собирает информацию о всех других маршрутизаторах и связях (линках) между ними. На основе этой информации он строит полную карту (топологию) сети и самостоятельно, с помощью алгоритма Дейкстры (Shortest Path First), рассчитывает кратчайший путь до каждой точки назначения.
...
Характеристика	RIP (Distance-Vector)	OSPF/IS-IS (Link-State)
Знание о сети	Только информация от соседей	Полная карта всей топологии
Метрика	Количество хопов	"Стоимость" на основе скорости
Обновления	Вся таблица, периодически	Только изменения, по факту
...
Если существует несколько путей с одинаковой метрикой, маршрутизатор может использовать их все одновременно для балансировки нагрузки (ECMP - Equal-Cost Multi-Path), как показано на слайде 20.
...
Презентация выделяет два режима работы BGP:
...
Так как BGP — это протокол политик, а не просто "стоимости", он оперирует не одной метрикой, а целым набором атрибутов, которые прикрепляются к каждому маршруту. Эти атрибуты используются в процессе выбора наилучшего пути. Презентация делит их на транзитивные (передаются от AS к AS) и нетранзитивные.
...
Презентация на слайде 31 приводит упрощенный, но очень показательный порядок этого алгоритма:
...
Как показано в презентации, доменные имена имеют иерархическую структуру, которая читается справа налево. Рассмотрим пример host.domain3.domain2.TLD:

. (Точка) — Корень (Root): Невидимый корень всей системы DNS.

TLD (Top-Level Domain) — Домен верхнего уровня: Это самая правая видимая часть имени. Презентация делит их на три категории:
...
1. Типы DNS-серверов
Презентация на слайдах 12-17 подробно описывает различные типы серверов, которые можно классифицировать по их роли и функциям.
...
Презентация на слайдах 24-27 наглядно демонстрирует пошаговый алгоритм разрешения доменного имени. Рассмотрим его на примере запроса A spb.auto.ru ?.
